...
const Team = () => {
  ...
  return (
    <div>
      <Layout>
        <div className="grid grid-cols-2 p-3 md:grid-cols-3">
          <Title>Equipe</Title>
          <Searchbar
            name="search"
            className="col-span-2 h-8 place-self-center md:col-span-1"
            placeholder={"Digite o nome de um integrante"}
            value={search}
            onChange={(event) => setSearch(event.target.value)}
          />
          <Filter
            className="order col-start-2 row-start-1 place-self-center md:hidden"
            name="role"
            id="filter"
            options={options}
            onChange={(event) => {
